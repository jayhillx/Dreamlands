...
public class ShrubBlock extends BushBlock implements BonemealableBlock {
    private static AbstractTreeGrower TREE_GROWER;

    public ShrubBlock(Properties properties) {
        super(properties);
    }

    public ShrubBlock(AbstractTreeGrower grower, Properties properties) {
        super(properties);
        TREE_GROWER = grower;
    }
    ...
    public VoxelShape getShape(BlockState state, BlockGetter getter, BlockPos pos, CollisionContext context) {
        return Shapes.or(Block.box(0.0D, 8.0D, 0.0D, 16.0D, 16.0D, 16.0D), Block.box(6.0D, 0.0D, 6.0D, 10.0D, 8.0D, 10.0D));
    }
